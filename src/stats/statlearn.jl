...
Distance-weighted discrimination loss function (smoothed `l1hingeloss`) with smoothing parameter `q`.
Loss is calculated between a boolean response `y ∈ {-1, 1}` and linear predictor `xβ`.
"""
...
Fit a model (via stochastic approximation) that is linear in the parameters.
